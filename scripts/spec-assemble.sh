...
# decision-map view (Task 8, scripts/spec-decisionmap.sh; D-2, REQ-C1.4), and the
# teach-back view (Task 5, scripts/spec-teachback.sh) over a bundle and emits a
# single self-contained HTML document on stdout:
#
#   - the foregrounded read first (the one-pager, then the decision map), the
#     teach-back prompt after it (silent-read-first ordering, REQ-D1.2);
...
# installed: it opens offline in any browser (D-4, REQ-E1.2). The dependency-graph
# diagram view and partial-scope rendering land in later tasks and extend this
# same assembly; this artifact carries the one-pager, the decision map, and the
# teach-back.
#
...
for s in "$onepager_sh" "$decisionmap_sh" "$teachback_sh"; do
  if [ ! -x "$s" ]; then
