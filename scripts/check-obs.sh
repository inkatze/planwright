...
# only to finding messages on stderr, so it strips C1 (0x80-0x9F) too — otherwise
# byte 0x9B (8-bit CSI) and friends survive as a live terminal-injection vector
# even after ESC (0x1B) is neutralized. The cost is that a legitimate multibyte
