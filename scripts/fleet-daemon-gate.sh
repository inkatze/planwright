...
#   exit 1 = paused (switch true) — the caller must short-circuit
...
#   exit 5 = broken install (the resolver cannot run at all): blocked
#
...
value=""
rc=0
value=$("$resolver" --key fleet_daemon_pause --type enum --values 'true false' --fallback false) || rc=$?
if [ "$rc" -ne 0 ]; then
