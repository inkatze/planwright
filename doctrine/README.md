...
| [orchestration-modes.md](orchestration-modes.md) | `/orchestrate`'s rare mode branches, read at point of use: the degradation ladder and runtime failover (degrade capability, never safety), the meta-tower (`--meta`: fleet lock, live-count bound, subordinate independence), and the fleet entry (`--fleet`: two-seam presentation, detached multiplexer plumbing, the attention surface) | orchestration-fleet REQ-B1.5, REQ-B1.6, REQ-D1.1, REQ-D1.5, REQ-E1.1, REQ-E1.2, REQ-E1.5 · orchestration-fleet D-3, D-6, D-9, D-12, D-13 |
