...
| Locks | `scripts/fleet-state.sh lock` for the cross-spec store; the supervisor's own elections (`journal.lock`, `recover.lock`, `launch.lock`) — all of them through the one primitive, `scripts/lock-lib.sh` | released by the holder on the normal path; on the abnormal path a break on positive evidence that the recorded holder is gone, so a killed holder cannot wedge the verb permanently; a hold that spans processes records no owner to probe and is recovered by an explicit `unlock` instead; the rung's `stop` releases the whole class as a second close | the recorded holder's liveness, and nothing else — no age anywhere (every lock in the class is a symlink whose target is the owner token) |
