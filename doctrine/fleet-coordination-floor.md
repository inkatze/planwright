...
| Locks | `scripts/fleet-state.sh lock` for the cross-spec store; the supervisor's own atomic `mkdir` elections (`journal.lock`, `recover.lock`) | released by the holder on the normal path; on the abnormal path a stale-age break at a documented bound, so a killed holder cannot wedge the verb permanently | the lock entry's own age against that same bound (the cross-spec lock is a symlink, the supervisor's elections are directories) |
