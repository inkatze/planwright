...
| Locks | `scripts/fleet-state.sh lock` and the supervisor's elections (`journal.lock`, `recover.lock`, `launch.lock`), all through one primitive, `scripts/lock-lib.sh` | released by the holder on the normal path; on the abnormal path a break on positive evidence that the recorded holder is gone, so a killed holder cannot wedge the verb permanently; a hold spanning processes records no owner to probe and takes an explicit `unlock`; the rung's `stop` releases the whole class as a second close | the recorded holder's liveness, and nothing else — no age anywhere |
| Scratch temp | created under the worker's state directory across its life: the stdio fifos, the staging temps each writer makes beside its target, and a lock break's claim and aside | its own class in the rung's `stop`, released after the process tree rather than with it, so a tree that will not close does not cost a live worker its channel; the captured result is durable record and is kept | residue under the state directory of a worker whose session has ended |
...
| `headless-oneshot` | per contract; a detached one-shot, closed the same way; gap: the `stop` arm does not ship yet — `scripts/fleet-dispatch-headless.sh` advertises `launch` and `status` and no close verb, which is a later task in this bundle | — no window | per contract (store locks only) | per contract; when the close lands, the captured result is durable record here too and is kept — REQ-B1.4's release set does not name it, and on this rung it is the whole point of the run | per contract |
| `subagent` | trivial — in-harness, no separate OS process; it dies with the tower | — no window | per contract — it runs the same scripts, so it takes the same store locks and is subject to the same owner-absence break | trivial — no worker state directory | trivial — the tower owns the row |
