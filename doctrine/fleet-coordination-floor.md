...
| --- | --- | --- | --- |
| Process tree | the rung's launch, recording the supervisor and worker pids under the worker's state directory and the record written at the dispatch seam | the rung's `stop` — SIGTERM then SIGKILL after a bounded grace, matched on the worker's state-directory path and never a bare process name; autonomously, `fleet-cleanup.sh process` | `scripts/fleet-death-evidence.sh`'s positive verdict, feeding the four-state classifier (`working` / `waiting-on-a-human` / `finished-but-unreaped` / `dead`) |
