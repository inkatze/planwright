...
REQ-B1.1, REQ-D1.6 · human-gates D-1, D-2, D-3, D-10 · supersedes bootstrap
D-26 (scoped), bootstrap REQ-J1.1, bootstrap REQ-J1.4.
...
| Force-pushing any branch | Human gate | Irreversible arm; only on the operator's explicit request in the same turn, executed by the tower after naming the remote commits it drops |
...
| The draft→ready flip of a unit PR | Policy | `ready_flip_policy`, default `unit-owner`; `human` reserves it |
...
| Pushing the unit branch and opening its draft PR | Composition contract | `/execute-task`, or a standalone `/self-review`; a `--nested` review skill never pushes |
...
the force-push gate above. A doc stating this floor cites this section; it
...
of reviewers and is one `--undo` from reversed. The checklist mechanics live
...
this doc where the list changed; its unchanged rules are the kickoff row
above. Its merge floor now reads as the floor section; its rewrite and
all-drafts rules now read as the rewrite gates, the rewrite policy, and the
flip policies.
