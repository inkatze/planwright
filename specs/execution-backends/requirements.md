...
- **REQ-A1.2** The contract SHALL define `headless-oneshot` (`claude -p` one-shot) as a backend
  value with an advertised capability set grounded in the verified research.
  *(Cites: D-3, obs:3414579b.)*
- **REQ-A1.3** The contract SHALL define `stream-json-persistent` as a session-grade backend
  ...
  `--resume` recovery, structured observe (event stream) and steer (message-in) surfaces.
  *(Cites: D-4, D-5, obs:3414579b.)*
...
- **REQ-A1.7** The pluggable-adapter `advertise` line grammar SHALL grow to carry the new fields
  back-compatibly: a legacy six-field line remains valid with fail-safe defaults, and a malformed
  line SHALL fail closed. *(Cites: D-13.)*
...
- **REQ-B1.1** `dispatch_backend` SHALL gain the semantic value `full-session`, resolving at
  ...
  REQ-B1.4), and the shipped default SHALL flip from `subagent` to `full-session` (a declared
  departure from the default-preserving rule, operator-scoped). *(Cites: D-8, obs:3414579b.)*
...
- **REQ-B1.4** Backend degradation SHALL degrade capability, never safety, and unattended
  ...
  orchestration-fleet REQ-B1.4 and orchestration-fleet D-3). *(Cites: D-8, orchestration-fleet
  REQ-B1.4 (Sources).)*
...
- **REQ-C1.4** When a petition under-determines the rung, `/offload` SHALL ask the operator and
  SHALL NOT silently guess. *(Cites: D-1, obs:3414579b.)*
- **REQ-C1.5** `/offload` SHALL report the dispatched worker's handle and how to observe or attach
  to it. *(Cites: drafting-session decision (2026-07-21).)*
...
- **REQ-F1.1** Fleet idle/busy detection SHALL treat `claude agents --json` as the authoritative
  busy/blocked oracle when available, demoting pane-scrape heuristics to fallback-only.
  *(Cites: D-11, obs:3414579b.)*
...
- **orchestration-fleet** (`specs/orchestration-fleet/`, Done) — owner of the backend seam this
  ...
  `dispatch_backend` knob. Foreign IDs cited from it are namespace-qualified.
...
- **Worktree config-layer gap** (recorded observations, 2026-07-15/2026-07-20, unconsumed): the
