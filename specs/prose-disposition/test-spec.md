...
suppression forms, audit and code-invariance modes, the PR-only CI step,
and the repository's existing index, link, options, anchor-freshness, and
instruction-budget checks, all run by `mise run check` and the repository
CI); `[Gherkin]` for the routing scenarios the amended doctrine must
produce, walked against the text at Task 1 with each walk recorded in the
PR body, and exercised on the first review pass after the skills
instantiate them; `[manual]` for the reading-dependent halves of the
cleanup verification and for one-time commands the worker runs and records
in the PR body for the reviewer to re-run; `[design-level]` where a rule's
presence in the named doc is the verification.

## REQ-A — Documentation lens defect class

### REQ-A1.1 — Documentation lens defect classes [design-level + Gherkin]

The Documentation lens entry in `doctrine/discovery-rigor.md` names the four
...
falsified class is recorded. Scenario: given a diff whose new flag's usage
text two readers would read differently, when the lens walks, then a
finding of the interpretation-fork class is recorded. Scenario: given a
diff whose only documentation
...
meaning-class test for prose findings with the meaning-based
normative-statement definition and the word list that aids the
preservation check, and cites the meta-spec's amendment axis rather than
redefining it.
...
a checklist entry is created. Scenario: given a PR that renames a doctrine
doc and changes a rule in it, when routed, then the doc is pre-existing
under rename detection and the rule change routes to Needs sign-off.
...
batched pass (the one REQ-C1.1's manual entry names) renders one entry per
batch with its sub-items.
...
one, still carrying its manifest line); a fix that edits a flag and its
usage text together is the code fix's own commit, not a batch member.
...
sharing a commit and checklist id. Manual: the first batched pass's (the
one REQ-C1.1's manual entry names) audit table has one row per finding.
...
The Task 2 PR body lists the surface patterns searched and the grep
command; a repository grep for the superseded phrasings returns hits only
in spec changelogs and observation fragments, run by the worker at Task 2,
recorded in the PR body, and repeatable by anyone.
...
`check:doctrine-index` passes with the new row; the guard's test file
asserts that the guard's header names the doc; the guard run over its own
file reports its largest block under the warn threshold, in `mise run
check`.

### REQ-E1.5 — Where the link lives instead [design-level + manual]

The doc names the commit trailer and the bundle deliverables as the link,
the two-part link test, and the deliverables-gap rule. Manual: the Task 9
closeout recomputes the unlinked set by that test, records the command and
its empty result in the PR body, and the reviewer re-runs it.
...
warns with exit zero, one below does not, one exactly at the error
threshold errors with a non-zero exit; the shebang is not counted; a blank
line and a code line each end a run; the reported starting line matches
the fixture.

### REQ-F1.2 — Surface and the heredoc limit [test]

Fixtures under each surface directory and a root-level fixture
(`lefthook.yml`, `mise.toml`, a workflow file) are measured; a fixture
outside the surface, and a non-shell non-YAML file inside a surface
directory (a `.txt` suppression list, a JSON file), are not; a heredoc body
line beginning with `#` is counted and the test asserts that documented
...
suppresses it and `--closeout` errors on it whether or not the file still
trips; a reason-less entry and a malformed entry each fail loud; outside
closeout an entry whose file no longer trips emits the named cleanup
warning and exit stays zero (the signal that an earlier cleanup PR removed
the need, never an error).
...
`--audit <path>` over a fixture tree emits the per-file rows ranked by
largest block, the corpus totals, words per block, and each block's
evidence class from the stated patterns, counting only blocks of two or
more lines; the Task 4 test materializes `scripts/*.sh` of the `v0.36.0`
tag into a temporary directory and asserts the pattern-independent
headline figures in Sources, and the emitted largest block and
evidence-class counts are recorded in the Task 4 PR body as the baseline
of record.

### REQ-F1.6 — Code invariance [test]

`--code-invariant` against a fixture base passes when only comment lines
and blank lines changed, fails naming the file when one code line changed,
fails naming the file when a surface file was added, and compares against
the merge-base so a code change merged into the base after the branch
point does not fail the branch.

### REQ-F1.7 — Wiring [test]

`check:comment-budget` appears in the `check` aggregate's dependency list
(the same assertion shape the existing wiring tests use), the `lefthook`
job is scoped to the surface globs, and the workflow file carries the
PR-only `--code-invariant` step conditioned on the `comments` title scope
(a grep-based case in the guard's test file over `.github/workflows/ci.yml`,
the shape the observation and githooks wiring tests use); `mise run check`
passes with the transitional baseline in place.
...
Test: the CI `--code-invariant` step passes on the Phase A PR and
`mise run check` is green with the reduced transitional baseline. Manual:
the PR body carries before and after audit figures, the kept-block list
whose length equals the after-audit's evidence-free count, and the
independent pass's record over a sample of at least ten kept blocks across
at least three files (or all if fewer) with every disagreement resolved.
...
### REQ-G1.4 — Comments only [test + manual]

Test: the PR-only CI step runs `--code-invariant` against the merge-base
on every PR whose title scope is `comments` and passes, proving no code
line in a surface file changed; `lint:shell` and `lint:fmt` pass. Manual:
each Phase A and Phase B PR's title carries the scope and its body records
the invariance result; the closeout PR carries no such scope.
...
line, the reason of every surviving block, the destination of every
relocation, and the reason of every `exempt` entry, with every
disagreement resolved (line restored or record amended) before the
operator's merge.
...
### REQ-G1.7 — Closeout [test + manual]

Test: `mise run check` passes with `--closeout` and zero `pending-cleanup`
entries. Manual: the closeout PR body records the final figures beside the
Task 4 baseline of record and lists every `exempt` entry with its reason.
