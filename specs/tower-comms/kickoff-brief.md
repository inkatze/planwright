...
| 24 | The attention marker is a contract two parallel tasks share: Task 3's `next` reads `<fleet home>/tower-comms/attention/<tower>` (one line holding the epoch of the operator's last reply, owner-only, mode-verified before the read) and Task 2's prompt-submit hook writes it; both were dispatched from the same base, so the hook could land with another path or shape (appended by the Task 3 run, 2026-09-15) | The read contract is stated once, in the script header and `read_marker`; the Task 2 worker takes it from there. Early signal: `next` reporting "no attention marker" in a session where the hook is installed |
| 25 | `tower_catchup_limit` (the settling task's knob) and `tower_shelve_return` (the delivery task's, D-9) landed with Task 3: store retention needs a bound at every write, `shelve` reads its return span, and a knob absent from every layer warns on every verb call until its owner ships it (appended by the Task 3 run, 2026-09-15) | The defaults and their options rows carry the D-3/D-6 semantics (closed records kept and catch-up entries rendered; the shelve return span), and the retention bound is capped at 500 because the retention pass sorts inside the fleet lock; Tasks 4 and 5 adjust the values or wording if the catch-up or the away model needs it. Early signal: the Task 4 or Task 5 worker finding the key present |
...
- **Task 3's three recorded deviations ratified, not folded in.** Risk rows
  25 to 27 stand as the record of what Task 3 shipped against what its block
  describes; the block is unchanged on that count. All three are
  ...
  blocks; and row 27's gap is closed by PR #471, where `run_store_pass` runs
  the settling pass before any verb acts, so amending Task 3 to say `next`
  does not settle would encode a state that is no longer true.
...
- **The Task 3 bullet removed from `## Awaiting input`.** All four parked
  ...
  of which tower held an item when it closed. The section is now empty;
  Task 8's wait on the Task 2 baseline is carried by its `Dependencies:`
  edge, never by a bullet, and is untouched.
...
guard, which strips `0x80`-`0x9F` as C1 controls and so refuses em dashes
and smart quotes, is a bug against the script's own field-grammar comment
and goes to a fix branch; no requirement mentions control bytes.
...
| Documentation | none | One residual: `main`'s `tower_catchup_limit` options row still reads "how many the catch-up renders", which PR #471 corrects to name the settled population. Closes on that merge, not here. |
