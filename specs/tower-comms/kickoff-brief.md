...
| 25 | `tower_catchup_limit` is the settling task's knob (D-9) but landed with Task 3: store retention needs a bound at every write, and a knob absent from every layer warns on every verb call until its owner ships it (appended by the Task 3 run, 2026-09-15) | The default and its options row carry the D-3/D-6 semantics (settled records kept, catch-up entries rendered); Task 4 adjusts the value or wording if the catch-up needs it. Early signal: the Task 4 worker finding the key present |
