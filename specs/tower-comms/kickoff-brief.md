...
| 24 | Research record, Task 2 (the prompt-submit hook is a version-sensitive, untrusted-input surface): the hooks reference (consulted 2026-09-15) reads exit 2 on `UserPromptSubmit` as "block and erase the prompt", injects plain stdout on exit 0 as model context, sets a 30 s default command timeout, and runs matching hooks in parallel; the payload key set read out of the CLI binaries on disk (2.1.271 to 2.1.273) is the shared envelope plus `prompt` and `session_title`, and that envelope has grown `agent_type` (always) and `prompt_id`, `scratchpad_dir`, `agent_id`, `effort` (when set) since the other fixtures were pinned | Decided: the hook exits 0 on every path, signals included, prints nothing on stdout, reads a 64 KiB prefix with its own awk walker, uses only `session_id`, `cwd`, `prompt` and the optional `prompt_id` (carried on the reply line when it is a UUID, dropped otherwise), validates the identities and the path against their grammars, and flattens the prompt to one bounded printable line before the log verb redacts it. Sources: the hooks reference page, the CLI binaries' payload construction sites, `fleet-liveness.sh`'s hook arm as the sibling pattern. Early signal: a `hook error` notice in a tower transcript, or a prompt that vanishes on submit |
| 25 | Live-worker definition for the tick (Task 2; a decision the brief left to execution, recorded per the drift check): "at least one live worker" names no source | Decided, low-stake and reversible (one awk expression in `scripts/tower-loop-log.sh`): a worker is live when its attention-store row is in working, idle, hung, awaiting-input or pr-ready; ended, merged, done and unparseable rows count nothing; a backend that never heartbeats (print) is invisible to the tick. Alternatives weighed: the registry's death handles (positive liveness, but the stream-json rung's handle column is not yet populated) and `fleet-status.sh merge` (probes the CLI oracle on every loop pass). Early signal: fleet hours at zero while the dashboard shows workers, or hours accruing from a stale `working` row after a crash |
| 26 | The attention marker is an interface built on two branches at once: the hook (Task 2) writes it, `next` (Task 3) reads it | Matched by reading Task 3's in-flight header at execution time: `<fleet-home>/tower-comms/attention/<tower-id>`, one line, the reply epoch in whole seconds (the knock and hand-over times it is compared against are whole seconds, so a finer value would read as later than a same-second hand-over that came after it), never moved backwards, owner-only, written lock-free by rename. The tower id is the session UUID: the hook keys the marker and its `reply` lines by the payload's session id and gates on the presence record of that name, so a tower must publish its presence with `--session-id`; one published by `--pid` (the composite identity) gets no marker and no reply lines, and a tower whose exported `PLANWRIGHT_TOWER_ID` differs from its session id splits its own lines across two identities in the log. Early signal: `next` reporting no marker in a session where the hook has written one, or the scorecard counting one tower as two |
| 27 | Baseline entry (Task 2), status at execution: not recordable yet | Only a tower running with this task's hook and tick writes the log, no session had them when Task 2 executed, and D-15 forbids reconstruction from transcripts. Row 16 stays reserved for the entry; the unit is parked in `tasks.md` Awaiting input with the operator's steps (a handful of sessions on a tower carrying the change, `mise run tower:report` at the end of each session, since the report has no session filter, and once over all of them). Task 8 waits for it |
| 28 | Review findings the Task 2 pass recorded rather than applied | (a) Verify-or-refuse gap, zone-paused per the gate-wiring pause protocol: the hook checks the `tower-comms/` and `attention/` directories and the marker file but not the fleet home above them, which `tower-queue.sh`'s `check_home` refuses when symlinked, group- or other-writable, or foreign-owned; recommended fix is the same three checks ahead of the sub-surface checks, declining the marker on refusal. (b) The log verb's private-key redaction replaces the `BEGIN … PRIVATE KEY` header only, so a pasted key body reaches `events.log`; the rule lives in `tower-queue.sh` (Task 1's, edited in flight by Task 3) and should span to the `END` line or drop the value. (c) The default 2 s lock wait is paid on the operator's prompt whenever the fleet lock is contended (measured 2.2 s against a held lock, 0.2 s uncontended); the bound is the kickoff's, recorded as an observation for the spec owner. Early signal for (a): a marker written where `log` refused the same home |
| 29 | Task 2 review gauntlet after execution (2026-09-16): the panel pass ran on the gemini backend, and the Copilot pass could not run | Panel (`/panel-review --nested`, three diff slices so the backend did not blank on size): nothing auto-applicable; one item recorded rather than applied, because the nested loop applies only tool-cited fixes: `tests/test-tower-loop-log.sh` asserts each identity source alone and never the precedence between them (`--tower` over `PLANWRIGHT_TOWER_ID`, that over `PLANWRIGHT_TOWER_SESSION_ID`); the fix is two cases in the identity-resolution block. Everything else the backend raised was dropped on validation or was already on this branch (row 28a, the observation entries). Copilot: requested on PR #468 (timeline `review_requested` at 01:56:50Z, 2026-09-17); the review arrived after the ten-minute poll window with nine threads and three suppressed comments. Applied: the two UUID grammars now require a 32-hex residue as `fleet-presence.sh` does, the tick reads a duplicated handle by its last row (the store's upsert contract), `delivered` drains stdin past the cap so the producer is never broken, the oversize-payload test captures `PIPESTATUS` before any other command, and the two docs no longer read the reply hook as a quiet observer or its lock wait as costing the prompt nothing. Left unresolved on the PR: the marker race and the session-UUID route (both in `tasks.md` Awaiting input), row 28a, and the private-key redaction (row 28b, `tower-queue.sh`'s rule). Dismissed: the `fleet_hours` arithmetic (the test runs green). Second review (of the fixes): one thread, the identity-precedence cases the panel had also asked for, applied; Copilot closed the private-key thread itself on reading the reply. Its suppressed comments, surfaced rather than applied, each with a single fix for the operator: refuse a `cwd` or session id carrying a control byte instead of stripping it (the path the presence check authenticates should be the path supplied); refuse a gated payload with no `prompt` key rather than log it as an empty reply; refuse a symlinked attention store in the tick (no sibling reader does today, so this is a policy for all of them); and the walker's tolerance of trailing bytes and a missing comma after the first top-level object, declined, since a bounded-prefix reader cannot validate root closure and the presence gate is what admits a payload. Row 24's field list was stale and is corrected. Third review: no new threads; of its suppressed comments, applied: the two doc sentences now say the knob bounds the lock wait rather than the hook's whole cost, the tick refuses an unsearchable attention directory instead of reading it as an idle fleet, and the unreadable-store case runs only as a non-root user (root reads through mode 0000 and the tick would have skewed the scorecard case). The two threads left open are the parked choices above. Early signal: the ready-flip taken before a Copilot review of the PR's final head exists |
| 30 | The attention marker is a contract two parallel tasks share: Task 3's `next` reads `<fleet home>/tower-comms/attention/<tower>` (one line holding the epoch of the operator's last reply, owner-only, mode-verified before the read) and Task 2's prompt-submit hook writes it; both were dispatched from the same base, so the hook could land with another path or shape (appended by the Task 3 run, 2026-09-15) | The read contract is stated once, in the script header and `read_marker`; the Task 2 worker takes it from there. Early signal: `next` reporting "no attention marker" in a session where the hook is installed |
| 31 | `tower_catchup_limit` (the settling task's knob) and `tower_shelve_return` (the delivery task's, D-9) landed with Task 3: store retention needs a bound at every write, `shelve` reads its return span, and a knob absent from every layer warns on every verb call until its owner ships it (appended by the Task 3 run, 2026-09-15) | The defaults and their options rows carry the D-3/D-6 semantics (closed records kept and catch-up entries rendered; the shelve return span), and the retention bound is capped at 500 because the retention pass sorts inside the fleet lock; Tasks 4 and 5 adjust the values or wording if the catch-up or the away model needs it. Early signal: the Task 4 or Task 5 worker finding the key present |
| 32 | Lease release on positive death evidence and on a holder with no live presence is not in Task 3's `next`: both are evidence reads the settling pass gathers outside the lock (Task 4); today `next` releases on acknowledgement, shelve, settling, the backstop expiry, and an attended tower's preemption of an away holder (appended by the Task 3 run, 2026-09-15) | A dead holder's item returns at `tower_lease_interval` or sooner through preemption. Early signal: an item waiting the full backstop after its tower died |
| 33 | Task 3's deliverable has `next` running the settling pass before it selects, but the level-triggered pass is the settling task's (Task 4); today `next` applies only the derived part (shelve returns, lease expiry, attention-pointer refresh) before selecting, so an item whose closing condition is met by evidence stays open until Task 4 lands (appended by the Task 3 review pass, 2026-09-15) | The seam is in place: `next` runs the derived pass under the same lock the settling pass will use, and `settle <id> --reason` closes on evidence by hand meanwhile. Early signal: a question answered by a merge still knocking after Task 4 lands |
