...
| 8 | **T6 execution note (2026-07-12) — window-lock base-reading:** the lock check must evaluate `main`'s state, not the PR head, or the release PR (which bumps the version) trips its own lock and can never merge (the chicken-and-egg D-7 rejects). Realized in `release-window.yml` by base-reading: `pull_request.base.ref` / `merge_group.base_ref` / `github.ref` fallback. Researched (GitHub Actions events reference, community #40277): `merge_group.base_ref` is the full base ref (`refs/heads/main`) while `github.ref` on a merge_group event is the queue ref (`gh-readonly-queue/...`) that *includes* the PR — so naming `merge_group.base_ref` explicitly is load-bearing, and the required check must list the `merge_group` trigger or the queue stalls. Low stake, reversible (workflow-only). | Low stake, reversible. Signal: merge queue enabled but the release PR is blocked by its own window check (a head-read regression) |
