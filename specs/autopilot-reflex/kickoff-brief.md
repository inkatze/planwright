...
| 10 | **T10 gauntlet self-review hardening (2026-07-13) — window-lock-excluding CI gate:** the Stage-2 self-review found that arm's CI check read the *aggregate* `statusCheckRollup.state`, which is red by design once a release is pending (the untagged-window lock, T6). Firing publish the instant the merge was observed would hit publish's own CI gate before the merged commit's CI even started. Fixed: arm's inline verdict now reads the rollup *contexts* and EXCLUDES the window-lock / `release-window` check, requiring only the quality checks green — both at pre-validation (PR head) and in a new fire-time settle-poll on the merged commit before delegating to publish (Diego decision: the window lock gates merges, not the publish). Also hardened: the post-publish cross-check re-derives the tag from the merged commit's version (never the stale arm-time proposal), and HEAD is re-confirmed to be `main` before the fire-time fast-forward. `release-publish.sh` stays byte-for-byte unchanged; **end-to-end correctness depends on the matching publish-side exclusion (`fix/publish-ci-gate-window-lock`) landing**, since publish's unchanged aggregate gate still refuses on the red window lock. Reversible (arm-only + tests). | Low stake, reversible. Signal: the publish hotfix diverging from arm's exclusion semantics, or the `window-lock`/`release-window` check name being renamed without updating `WINDOW_LOCK_*` |
