...
| 9 | **T10 execution note (2026-07-12) — armed mode watches via the `gh` PR JSON API, delegates tagging to the unchanged publish:** `scripts/release-arm.sh` pre-validates (on-main/clean/synced, PR OPEN, proposed-version SemVer + monotonicity + no-existing-tag against origin, CI green on the PR head), then polls `gh pr view <n> --json state,mergeCommit` until `MERGED` (fire) or `CLOSED` (disarm) and on the merge delegates to `release-publish.sh`. Researched: `state`/`headRefOid`/`mergeCommit`/`statusCheckRollup` are all current `gh pr view --json` fields (verified against gh 2.93.0); `mergeCommit.oid` on a `MERGED` PR is the base-branch merge/squash commit, i.e. the observed merge SHA. Arm never weakens safety (D-12, REQ-E1.4): its pre-checks only earn the right to WATCH; publish re-enforces every gate and recomputes the release-merge SHA itself (D-6), so an arm-time→merge-time race is caught by publish, not slipped through. The proposed version is read from the PR head via a read-only `refs/pull/<n>/head` fetch (GitHub pull ref). `release-publish.sh`/`release-lib.sh` are byte-for-byte unchanged (the post-merge fallback); arm keeps an inline CI-rollup check rather than editing publish. Reversible (a new standalone script only). | Low stake, reversible. Signal: a `gh` JSON field rename on a future bump, or a `refs/pull/*/head` fetch refusal on a non-GitHub forge |
| 10 | **T10 gauntlet self-review hardening (2026-07-13) — window-lock-excluding CI gate:** the Stage-2 self-review found that arm's CI check read the *aggregate* `statusCheckRollup.state`, which is red by design once a release is pending (the untagged-window lock, T6). Firing publish the instant the merge was observed would hit publish's own CI gate before the merged commit's CI even started. Fixed: arm's inline verdict now reads the rollup *contexts* and EXCLUDES the window-lock check, requiring at least one quality check green (positive confirmation) — both at pre-validation (PR head) and in a new fire-time settle-poll on the merged commit before delegating to publish (Diego decision: the window lock gates merges, not the publish). Also hardened: the post-publish cross-check re-derives the tag from the merged commit's version (never the stale arm-time proposal), and HEAD is re-confirmed to be `main` before the fire-time fast-forward. **Update (2026-07-13 gauntlet Stage-3):** the matching publish-side exclusion landed as **#163** (merged to main; `release-publish.sh`'s `ci_green` now evaluates per-check `contexts` with the window lock excluded, no longer the aggregate `state`), so end-to-end armed release works; arm's exclusion predicate was then aligned to publish's *exactly* (only the CheckRun named `window-lock`, a legacy StatusContext of the same name is judged) and arm's verdict gained publish's positive-confirmation and >100-check (`TOO_MANY`) fail-closed handling. `release-publish.sh` stays byte-for-byte unchanged in arm's own commits (the shared exclusion helper is a noted follow-up). Reversible (arm-only + tests). | Low stake, reversible. Signal: arm's exclusion predicate diverging from publish's, or the `window-lock` check name being renamed without updating `WINDOW_LOCK_NAME` |

No open questions: row 2 is an explicit accepted risk, row 3 is a recorded
decision, rows 7–10 are recorded execution notes (T5 dependency-adoption, T6
window-lock base-reading, T10 armed-mode watch/delegate, T10 gauntlet
window-lock-excluding CI gate), all rows carry mitigation and early signal. Data
...
prior entry's commit (`f86629a`) yields the same hash recorded below, so no
...
Class: expression-only
Anchor: `f38cfee6408b567ee0aee72fab3edaeca200cb31` — computed as
`scripts/spec-anchor.sh specs/autopilot-reflex`
