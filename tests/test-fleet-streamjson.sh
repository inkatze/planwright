...
# init + one pending permission request, then the worker holds open.
ev_hold="$tmp/ev-hold"
...
# being searched for) is never in the snapshot it is searching. Mirrors the
# script's own degradation to the narrow form when `ps` rejects `-ww`.
ps_rows() {
