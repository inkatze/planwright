...
# 4. process: a live process is alive (exit 1).
sleep 30 &
live_pid=$!
...
# 10. tmux binary absent entirely -> unknown (no observability at all).
rc=0
out=$(PATH="$tmp/empty-bin:/usr/bin:/bin" /bin/bash "$FDE" tmux-window planwright worker-3 2>/dev/null) || rc=$?
