...
#   - the push+PR section is serialized by the shared advisory lock
...
# --- lock fixtures -----------------------------------------------------------
# The carry's advisory lock (scripts/lock-lib.sh) IS a symlink whose target is
# the holder's `<pid>-<epoch>-<seq>` token, so a test can stand one up by hand:
# point it at a process it controls and the carry reads exactly the liveness the
# library reads. `run_carry` puts the state dir at $ghstate/carry-lock.
lock_path_for() { printf '%s/carry-lock/observation-carry.lock' "$1"; }

# Plant a lock owned by a LIVE process and print that process's pid. The caller
# kills it (the fixture deliberately outlives the carry under test).
plant_live_lock() {
  _pl_state="$1"
  mkdir -p "$_pl_state/carry-lock"
  # The redirect is load-bearing: a background job left holding the command
  # substitution's pipe keeps `holder=$(plant_live_lock ...)` blocked until the
...
# ---------------------------------------------------------------------------
# Case 18 — STALE MEANS THE OWNER'S PROCESS IS ABSENT. A lock whose owner has
# exited is broken and re-acquired even though its mtime is seconds old, so a
# carry killed mid-run never wedges the next one (the age rule this replaces
# would have left it standing for the whole threshold).
# ---------------------------------------------------------------------------
c18() {
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/obs-carry.c18.XXXXXX")
  trap 'rm -rf "$tmp"' RETURN
  repo="$tmp/repo"
  seed_repo "$repo"
  gitc "$repo" checkout -q -b planwright/fleet-hardening/task-9
  add_frags "$repo" tower qqqq2222
  gh="$tmp/bin"
  make_gh_stub "$gh"
  plant_dead_lock "$tmp/ghstate"
  lock=$(lock_path_for "$tmp/ghstate")
  [ -L "$lock" ] || fail "c18: fixture did not plant a lock symlink"

  out=$(run_carry "$gh" "$tmp/ghstate" "$repo" 2>"$tmp/err") \
    || fail "c18: carry exit $? — $(cat "$tmp/err")"
  [ "$(tag_val "$out" carry)" = created ] \
    || fail "c18: a dead owner's lock must be broken and the carry proceed, got: $out"
  [ ! -e "$lock" ] && [ ! -L "$lock" ] \
    || fail "c18: the carry must release its own lock on the way out"
  echo "ok c18: a lock whose owner process is absent is stale and is broken"
}

# ---------------------------------------------------------------------------
# Case 19 — AGE IS NOT STALENESS. A lock back-dated to the year 2000 whose owner
# is STILL RUNNING must not be broken: the old `find -mmin +N` rule would have
# broken it and let two carries into the push+PR section at once.
# ---------------------------------------------------------------------------
c19() {
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/obs-carry.c19.XXXXXX")
  trap 'rm -rf "$tmp"' RETURN
  repo="$tmp/repo"
  seed_repo "$repo"
  gitc "$repo" checkout -q -b planwright/fleet-hardening/task-9
  add_frags "$repo" tower rrrr3333
  gh="$tmp/bin"
  make_gh_stub "$gh"
  holder=$(plant_live_lock "$tmp/ghstate")
  lock=$(lock_path_for "$tmp/ghstate")
  # Back-date the LINK itself (-h: never follow it to the target).
  touch -h -t 200001010000 "$lock" 2>/dev/null \
    || echo "note c19: touch -h unsupported here; the liveness assertion still holds"
  token_before=$(readlink "$lock")
  ...
  [ -L "$lock" ] || fail "c19: the live holder's ancient lock was removed"
  [ "$(readlink "$lock")" = "$token_before" ] \
    || fail "c19: the live holder's ancient lock was broken and re-taken (age is not staleness)"
  [ ! -s "$tmp/ghstate/create-log" ] || fail "c19: must open no PR while another carry holds the lock"
  echo "ok c19: an ancient lock whose owner is still running is NOT broken"
}

# ---------------------------------------------------------------------------
# Case 20 — a concurrent second carry is cleanly REFUSED while a live holder has
# the lock: no push, no PR, exit 0 — and the refusal is not a drop, so once the
# holder is gone the very same set is carried.
# ---------------------------------------------------------------------------
c20() {
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/obs-carry.c20.XXXXXX")
  trap 'rm -rf "$tmp"' RETURN
  repo="$tmp/repo"
  seed_repo "$repo"
  gitc "$repo" checkout -q -b planwright/fleet-hardening/task-9
  add_frags "$repo" tower tttt4444
  gh="$tmp/bin"
  make_gh_stub "$gh"
  holder=$(plant_live_lock "$tmp/ghstate")
  lock=$(lock_path_for "$tmp/ghstate")
  ...
  [ "$(tag_val "$out" stranded)" = 0 ] \
    || fail "c20: a contended no-op reports stranded=0 (the holder carries the set), got: $out"
  [ ! -s "$tmp/ghstate/create-log" ] || fail "c20: contended run must open no PR"
  gitc "$repo" ls-remote --heads origin "planwright/chore/observations" 2>/dev/null | grep -q . \
    && fail "c20: contended run must push no chore branch"
  [ -L "$lock" ] || fail "c20: the contended run removed the live holder's lock"

  # The refusal deferred the work, it did not drop it: with the holder gone, the
  # same fragment is carried.
  out2=$(run_carry "$gh" "$tmp/ghstate" "$repo" 2>"$tmp/err") \
    || fail "c20: post-contention carry exit $? — $(cat "$tmp/err")"
  [ "$(tag_val "$out2" carry)" = created ] \
    || fail "c20: the deferred set must be carried once the holder is gone, got: $out2"
  echo "ok c20: a concurrent carry is cleanly refused while a live holder has the lock"
}

# ---------------------------------------------------------------------------
# Case 21 — a lock left as a DIRECTORY by the retired `mkdir` shape must not
# wedge the repo. Nothing releases one any more and the library refuses to wait
# on a non-symlink, so the carry clears it in place and proceeds.
# ---------------------------------------------------------------------------
c21() {
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/obs-carry.c21.XXXXXX")
  trap 'rm -rf "$tmp"' RETURN
  repo="$tmp/repo"
  seed_repo "$repo"
  gitc "$repo" checkout -q -b planwright/fleet-hardening/task-9
  add_frags "$repo" tower uuuu5555
  gh="$tmp/bin"
  make_gh_stub "$gh"
  lock=$(lock_path_for "$tmp/ghstate")
