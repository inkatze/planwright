...
assert_allow "tmux show-buffer (read-only buffer dump)" "tmux show-buffer -b relay"
assert_allow "tmux showb alias" "tmux showb"
assert_allow "tmux list-buffers observe" "tmux list-buffers"
...
assert_defer "deny git branch --force" "git branch --force main HEAD"
assert_defer "deny git update-ref" "git update-ref refs/heads/main HEAD"
assert_defer "deny git commit --fixup" "git commit --fixup HEAD"
assert_defer "deny git filter-repo" "git filter-repo --path x"
assert_defer "deny git push --mirror" "git push --mirror"
assert_defer "deny git push --all" "git push --all origin"
