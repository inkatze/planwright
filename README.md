...
shellcheck, shfmt, markdownlint, yamllint, manifest + link + options-reference
checks, the spec validator, and a gitleaks scan. It is the same gate CI runs on
every pull request. This is dev tooling only; planwright's runtime scripts stay
plain portable bash. See [Contributing](docs/CONTRIBUTING.md) for the workflow.
