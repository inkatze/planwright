...
| `dispatch_backend` | `subagent` | Which dispatch backend `/orchestrate` uses for execution units: `subagent` (default), `tmux`, `print`, or `in-session` (D-38, REQ-F1.8). Each value maps to an advertised capability set per the [backend capability contract](../doctrine/backend-capability-contract.md) (D-2), which defines how a new terminal/multiplexer is to become a selectable value by advertising the contract rather than by a skill edit (contract defined; dispatch wiring is a later orchestration-fleet task, so the dispatch layer is still backend-keyed today). | `/orchestrate` |
