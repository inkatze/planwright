...
| `PermissionRequest` | working → `awaiting-input`, queued, plus a pending-permission marker; the row carries the `permission` marker in field 9 and the prompt's own command text in field 12 |
